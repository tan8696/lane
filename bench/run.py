...
def run_one(task, arm, model=None, keep=False):
    ...
    if p.returncode == 124:
        ...
        error = "usage limit"
    elif p.returncode != 0 and not rows:
        error = f"agent exit {p.returncode}"
    result = {
        ...
        "seconds": secs,
        "agent_tail": (p.stdout or p.stderr or "").strip()[-300:],
...
def table(results):
    head = f"| {'task':9} | {'arm':8} | {'pass':5} | {'files':5} | {'lines':5} | {'off-intent':10} | {'sec':5} |"
    ...
    for r in results:
        ...
        out.append(f"| {r['task']:9} | {r['arm']:8} | {status:5} | "
                   f"{r['files']:5} | {r['lines']:5} | {r['off_intent_lines']:10} | {r['seconds']:5} |")
    return "\n".join(out)
...
def main():
    ...
    for t in tasks:
        for arm in arms:
            print(f"-> {t['id']} / {arm} ...", flush=True)
            r = run_one(t, arm, a.model, a.keep)
            print(f"   pass={r['passed']} lines={r['lines']} off-intent={r['off_intent_lines']} "
                  f"{r['seconds']}s {r['off_intent_files'] or ''}", flush=True)
            results.append(r)
